...
trait CRM {
    public static $VERSION = '1.36';
    public static $BATCH_COUNT    = 50;//count batch 1 query
    public static $TYPE_TRANSPORT = 'json';// json or xml
    public static $C_REST_WEB_HOOK_URL = "";  //url on creat Webhook
    public static $C_REST_CURRENT_ENCODING = 'unicode';
    public static $C_REST_CLIENT_ID = "YOUR_C_REST_CLIENT_ID";
    public static $C_REST_CLIENT_SECRET = "YOUR_C_REST_CLIENT_SECRET";
    public static $C_REST_IGNORE_SSL;
    public static $C_REST_BLOCK_LOG;
    public static $C_REST_LOGS_DIR;
    public static $C_REST_LOG_TYPE_DUMP;
    public static $AUTH_ID;
    public static $DOMAIN;
    public static $REFRESH_ID;
    public static $APP_SID;
        ...
        if($arSettings !== false)
        {
            ...
            else
            {
                if(!empty(self::$DOMAIN)){
                    $url = 'https://' . htmlspecialchars(self::$DOMAIN) . '/rest/' . $arParams[ 'method' ] . '.' . static::$TYPE_TRANSPORT;
    ...
    public static function call($method, $params = [])
    {
        ...
        if(!empty($result['error_description']) && empty($result['result'])) dd($result);
        // throw ValidationException::withMessages(['ERROR_METHOD_NOT_FOUND' => "Ошибка!"]); // Сделать нормальную обработку ошибок!
        else if(empty($result['result'])) $result; // Сделать нормальную обработку ошибок!
        else return $result;
    }
    ...
    public static function callBatch($arData, $halt = 0)
    {
        ...
        if(empty($arResult['result'])) dd($arResult['error_description']); // Сделать нормальную обработку ошибок!
        else return $arResult['result'];
    }
    ...
    {
        $return = [];
        $return['access_token'] = htmlspecialchars(static::$AUTH_ID);
        $return['domain'] = htmlspecialchars(static::$DOMAIN);
        $return['refresh_token'] = htmlspecialchars(static::$REFRESH_ID);
        $return['application_token'] = htmlspecialchars(static::$APP_SID);
        $return['client_endpoint'] = htmlspecialchars(static::$member_id);
    ...
    {
        $return = false;
        if(!defined("C_REST_BLOCK_LOG") || static::$C_REST_BLOCK_LOG !== true)
        {
            ...
            if(!defined("C_REST_LOG_TYPE_DUMP") || static::$C_REST_LOG_TYPE_DUMP !== true)
            {
    ...
    public static function InitialSetup($request)
    {
        static::$C_REST_WEB_HOOK_URL = "";
        static::$AUTH_ID = $request->get("token")["ACCESS_TOKEN"];
        static::$DOMAIN = $request->get("token")["DOMAIN"];
        static::$REFRESH_ID = $request->get("token")["REFRESH_TOKEN"];
        static::$member_id = $request->get("token")["MEMBER_ID"];
        static::$APP_SID = $request->get("sid")["APP_SID"];
    }
}
