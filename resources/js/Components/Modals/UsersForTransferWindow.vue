...
<template>
  <div v-if="show" class="modal-shadow" @click.self="closeModal">
    <div class="modal">
      ...
      <slot name="title">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Выбор сотрудника для передачи тикета</h2>
      </slot>
      ...
      <slot name="footer">
        <div class="modal-footer">
					<div>
						<button v-on:click="TransferTicket"
...
export default {
  ...
  methods: {
    closeModal: function () {
		...
		TransferTicket() {
			if (this.user_id == 0) {
				return this.$page.props.toast = {
					status: "error",
					message: "Необходимо выбрать сотрудника для передачи"
				}
      ...
      bx24.getAuth().then(auth => {
        axios.post("/api/ticket/transfer", {
          token: auth,
          sid: this.$parent.Parameters,
